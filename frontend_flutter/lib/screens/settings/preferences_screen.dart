...
class _PreferencesScreenState extends State<PreferencesScreen> {
  ...
  List<Map<String, dynamic>> savedLocations = [
    {'title': 'Home', 'subtitle': '123 University Ave, Block A', 'icon': Icons.home},
    {'title': 'Campus', 'subtitle': 'Main Square Gate 4, Sector 62', 'icon': Icons.school},
  ];
  ...
  Future<void> _handleLocationToggle(bool requestedState) async {
    if (requestedState == true) {
      ...
      if (!isGpsOn) {
        ...
        _showSimpleSnackBar('Location services are disabled. Please turn on your device GPS.');
  ...
  void _showAddressBottomSheet({int? index}) {
    ...
    String activeTag = ['Home', 'Work'].contains(initialTitle) ? initialTitle : 'Other';
    
    final flatController = TextEditingController(text: initialFlat);
    final areaController = TextEditingController(text: initialArea);
    final customTagController = TextEditingController(text: activeTag == 'Other' ? initialTitle : '');
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text(
                          isEditing ? 'Update Address' : 'Enter complete address',
                          style: TextStyle(
                            fontSize: 18,
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 12),
                        _buildTagChip('Other', Icons.location_on, activeTag, (tag) {
                          setModalState(() => activeTag = tag);
                        }),
                      ],
                    ),
                      ...
                      children: [
                        ...
                        Expanded(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              if (flatController.text.isNotEmpty && areaController.text.isNotEmpty) {
                                String finalTitle = activeTag == 'Other' 
                                    ? (customTagController.text.isEmpty ? 'Other' : customTagController.text) 
                                    : activeTag;
                                
                                IconData finalIcon = activeTag == 'Home' ? Icons.home 
                                    : (activeTag == 'Work' ? Icons.work : Icons.location_on);

                                setState(() {
                                  final newLocation = {
                                    'title': finalTitle,
                                    'subtitle': '${flatController.text}, ${areaController.text}',
                                    'icon': finalIcon,
                                  };
  ...
  Widget _buildTagChip(String label, IconData icon, String activeTag, Function(String) onTap) {
          ...
          children: [
            ...
            _buildContainer(
              child: _buildSwitchTile(
                ...
                subtitle: 'Allow fellow carpoolers to see your real-time pickup spot for 15 minutes before your scheduled ride.',
